...
if (isset($_POST['approve'])) {
  $sql = "UPDATE `user_order` SET `stat`='Approved' WHERE `orderid`='$id'";
...
<div class="container mt-5">
    <div class="row">
        ...
        <div class="col-sm-8">
          <a href="chef_home.php" class="btn btn-outline btn-primary">Back</a>
            <table class="table table-striped table-sm mt-3">
                ...
                <?php
                $sql = "SELECT * FROM user_order WHERE orderid = '$id' AND status='Approved'";
                $res = $conn->query($sql);
                    //$n=0;
                    if ($res->num_rows > 0) {
...
<?php
if (isset($_POST['approve'])) {
  $sql = mysqli_query($conn,"UPDATE `user_order` SET `stat`='Approved' WHERE `orderid`='$_POST[id]'");
  echo"<script>
    alert('Approved');
    window.location.href='chef_home.php';
    </script>";
}
?>
